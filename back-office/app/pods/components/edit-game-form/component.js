...
export default Component.extend({
  ...
  defaultDate: computed('model.release_date', function () {
    return this.get('model.release_date')
  }),
  dateValue: computed('model.release_date', function () {
    return this.get('model.release_date')
  }),
  authors: computed('model.authors', function () {
      return this.get('model.authors');
  }),
  tags: computed('model.tags', function () {
      const tags = [];
      this.get('model.tags').forEach(tag => {
        tags.push(tag.tag);
      });
    return tags;
  }),
